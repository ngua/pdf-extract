...
fn pdf_to_utf8(s: &[u8]) -> error::Result<String> {
...
fn to_utf8(encoding: &[u16], s: &[u8]) -> error::Result<String> {
            ...
            .and_then(|x| {
                ...
                match all {
                    ...
                    Ok(all) => {
                        ...
                        let n3 = all.next().ok_or_else(err)?;
                        let n4 = all.next().ok_or_else(err)?;
                        Ok([n1, n2, n3, n4])
                    }
                    Err(e) => Err(e),
                }
...
fn get_name_string<'a>(
    doc: &'a Document,
    dict: &'a Dictionary,
    key: &[u8],
) -> error::Result<String> {
    let n = dict.get(key)?;
    let md = maybe_deref(doc, n)?.as_name()?;
    pdf_to_utf8(md)
}
...
fn make_font<'a>(
    doc: &'a Document,
    font: &'a Dictionary,
) -> error::Result<Rc<dyn PdfFont + 'a>> {
    match get_name_string(doc, font, b"Subtype")?.as_str() {
        "Type0" => Ok(Rc::new(PdfCidFont::new(doc, font)?)),
        "Type3" => Ok(Rc::new(PdfType3Font::new(doc, font)?)),
        _ => Ok(Rc::new(PdfSimpleFont::new(doc, font)?)),
    }
}
...
fn encoding_to_unicode_table(name: &[u8]) -> error::Result<Vec<u16>> {
    let encoding = match name {
        b"MacRomanEncoding" => Ok(encodings::MAC_ROMAN_ENCODING),
        b"MacExpertEncoding" => Ok(encodings::MAC_EXPERT_ENCODING),
        b"WinAnsiEncoding" => Ok(encodings::WIN_ANSI_ENCODING),
...
impl<'a> PdfSimpleFont<'a> {
    fn new(
        ...
        let mut unicode_map = get_unicode_map(doc, font).ok();
        ...
        match encoding {
            ...
            Some(Object::Dictionary(encoding)) => {
                let mut table = if let Some(base_encoding) =
                    maybe_get_name(doc, encoding, b"BaseEncoding")
                {
                ...
                let differences =
                    maybe_get_array(doc, encoding, b"Differences");
                        ...
                        match o {
                            ...
                            Object::Name(n) => {
                                ...
                                if let Some(unicode) = unicode {
                                    ...
                                    if let Some(ref mut unicode_map) =
                                        unicode_map
                                    {
            ...
            None => {
                if let Some(type1_encoding) = type1_encoding {
                    ...
                    for (code, name) in type1_encoding {
                        let unicode =
                            glyphnames::name_to_unicode(&pdf_to_utf8(&name)?);
                        if let Some(unicode) = unicode {
                            table[code as usize] = unicode;
                        }
                    }
        ...
        // If we have widths prefer them over the core font widths.
        // Needed for https://dkp.de/wp-content/uploads/parteitage/Sozialismusvorstellungen-der-DKP.pdf
        if let (Some(first_char), Some(last_char), Some(widths)) = (
                ...
                if font_metrics.0 == base_name {
                    if let Some(ref encoding) = encoding_table {
                        ...
                        for w in font_metrics.2 {
                            // -1 is "not encoded"
                            if w.0 != -1 {
                                table[w.0 as usize] = if base_name
                                    == "ZapfDingbats"
                                {
                                    zapfglyphnames::zapfdigbats_names_to_unicode(w.2)
                                        .ok_or_else(|| OutputError::from(format!("bad name {:?}", w)))?
                                } else {
                                    glyphnames::name_to_unicode(w.2)
                                        .ok_or_else(|| {
                                            OutputError::from(format!(
                                                "bad name {:?}",
                                                w
                                            ))
                                        })?
            ...
            Err(OutputError::from("no widths"))?;
        }
    ...
    #[allow(dead_code)]
    fn get_type(&self) -> error::Result<String> {
        get_name_string(self.doc, self.font, b"Type")
    }
    #[allow(dead_code)]
    fn get_basefont(&self) -> error::Result<String> {
        get_name_string(self.doc, self.font, b"BaseFont")
    }
    #[allow(dead_code)]
    fn get_subtype(&self) -> error::Result<String> {
                ...
                if let Some(differences) = differences {
                    ...
                    for o in differences {
                        match o {
                            ...
                            _ => {
                                Err(OutputError::from("wrong type"))?;
                            }
                        }
                    }
                }
...
impl<'a> PdfFont for PdfType3Font<'a> {
    ...
    fn decode_char(&self, char: CharCode) -> error::Result<String> {
        ...
        if let Some(ref unicode_map) = self.unicode_map {
            let s = unicode_map.get(&char).ok_or_else(|| {
                OutputError::from(format!(
                    "missing char {:?} in map {:?}",
                    char, unicode_map
                ))
    ...
    fn new(doc: &'a Document, font: &'a Dictionary) -> error::Result<Self> {
        ...
        let encoding =
            maybe_get_obj(doc, font, b"Encoding").ok_or_else(|| {
                OutputError::from("Encoding required in type0 fonts")
            })?;

        let encoding = match encoding {
            Object::Name(_) => ByteMapping {
...
fn show_text(
    ...
    for (c, length) in font.char_codes(s) {
        ...
        let tx = ts.horizontal_scaling
            * ((w0 - tj / 1000.) * ts.font_size + spacing);
        ts.tm = ts
            .tm
        ...
        match name {
            ...
            _ => {
                ...
                if let Ok(cs) = cs.as_array() {
                    let cs_name = pdf_to_utf8(cs[0].as_name()?)?;
                    match cs_name.as_ref() {
                        "Separation" => {
                            let n = cs.get(1).ok_or_else(|| {
                                OutputError::from("second arg must be a name")
                            })?;
                            let n = n.as_name()?;
                            let name = pdf_to_utf8(n)?;
                            let alternate_space =
                                match &maybe_deref(doc, &cs[2])? {
                                    ...
                                    Object::Array(cs) => {
                                        ...
                                        match cs_name.as_ref() {
                                            "ICCBased" => {
                                                let stream =
                                                    maybe_deref(doc, &cs[1])?
                                                        .as_stream()?;
                                                AlternateColorSpace::IccBased(
                                                    get_contents(stream),
                                                )
                                            }
                                            "CalGray" => {
                                                let dict = cs[1].as_dict()?;
                                                AlternateColorSpace::CalGray(
                                                    CalGray {
                                                        white_point: get(
                                                            doc,
                                                            dict,
                        ...
                        "ICCBased" => {
                            let stream =
                                maybe_deref(doc, &cs[1])?.as_stream()?;
                            // XXX: we're going to be continually decompressing
                            // everytime this object is referenced
                            Ok(Self::IccBased(get_contents(stream)))
                        }
                        "CalGray" => {
                            let dict = cs[1].as_dict()?;
...
impl<'a> OutputDev for HtmlOutput<'a> {
    ...
    fn output_character(
        ...
        let transformed_font_size_vec =
            trm.transform_vector(vec2(font_size, font_size));
        // get the length of one sized of the square with the same area with a rectangle of size (x, y)
...
impl<'a> OutputDev for SvgOutput<'a> {
    fn begin_page(
        ...
        let ctm =
            Mat::create_scale(1., -1.).post_translate(vec2(0., media_box.ury));
        writeln!(
            self.file,
            "<g transform='matrix({}, {}, {}, {}, {}, {})'>",
...
impl<W: ConvertToFmt> OutputDev for PlainTextOutput<W> {
    fn begin_page(
        ...
        self.flip_ctm = Transform2D::row_major(
            1.,
            0.,
            0.,
            -1.,
...
fn maybe_decrypt(doc: &mut Document) -> error::Result<()> {
    if !doc.is_encrypted() {
        Ok(())
    } else {
        Ok(doc.decrypt("")?)
...
pub fn extract_text_from_bytes(buffer: &[u8]) -> error::Result<String> {
    let mut s = String::new();
    {
        let mut output = PlainTextOutput::new(&mut s);
...
pub fn extract_text_from_bytes_encrypted<PW: AsRef<[u8]>>(
    buffer: &[u8],
    password: PW,
) -> error::Result<String> {
    let mut s = String::new();
...
pub fn output_doc(
    ...
    if doc.is_encrypted() {
        let msg = "Encrypted documents must be decrypted with a password \
            using {{extract_text|extract_text_from_mem|output_doc}}_encrypted";
        Err(OutputError::from(msg))
    } else {
        ...
        for dict in pages {
            ...
            let resources = get_inherited(doc, page_dict, b"Resources")
                .unwrap_or(empty_resources);

            // pdfium searches up the page tree for MediaBoxes as needed
            let media_box: Vec<f64> =
                get_inherited(doc, page_dict, b"MediaBox")
                    .ok_or_else(|| OutputError::from("MediaBox"))?;
